...
**现象**：为可用性考虑，让缓存/Redis 不可用时「降级放行」，并认为「反正数据库唯一索引能兜住」。实际上这个兜底并不存在。

**错误推理**

> 「Redis 挂了不影响下单，重复提交由 `order_no` 唯一索引拦截。」

但 `order_no` 是**每次请求新生成的**（雪花 ID、时间戳 + 随机数），两次重复提交会产生两个不同的值，唯一索引永远不会冲突。

**正确做法**

幂等兜底必须建立在**业务唯一键**上，而不是「每次生成都不同」的技术主键：

1. 前端为每次提交生成 `requestId`（UUID），服务端在业务表建 `uk_request_id` 唯一索引；
2. 或用业务组合键（如 `user_id + 商品指纹 + 时间窗`）建唯一索引；
3. Redis 只作为**前置加速层**，数据库唯一索引才是最终防线；
4. 若确实无法建立业务唯一键，降级策略应改为「拒绝服务」而非「放行」—— 宁可让用户重试，不可产生重复数据。

**一句话**：技术主键的唯一性 ≠ 业务幂等。写下「最后一道防线是 XXX」之前，先验证 XXX 是否真的能拦住重复请求 —— 通常把重复请求跑一遍就能发现它拦不住。
...
| 循环里查数据库 | 改成批量查询 + 内存组装 |
| 缓存/Redis 降级放行 | 先确认真正能拦住重复请求的业务唯一键是什么，技术主键不算 |
